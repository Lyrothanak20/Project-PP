...
const generatePageNumber = () => {
  ...
  if (loading) {
    return (
      <div className="animate-pulse grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-6">
        {[...Array(6)].map((_, i) => (
          <div
            key={i}
          ...
          {paginatedBooks.map((book) => {
            ...
            return (
              <div
                id="hero"
                key={book.id}
                className="group relative border-y-gray-800/30 rounded-xl md:rounded-2xl backdrop-blur-xl border border-white/10 hover:border-cyan-400/30 transition-all duration-300 hover:translate-y-1 md:hover:translate-y-2 shadow-lg hover:shadow-xl md:shadow-2xl hover:shadow-cyan-400/10"
              >
                <a
                  href={book.infoLink}
                  target="_blank"
